...
class collapsible_widget_area {
	...
	function __construct() {
		...
		if ( is_multisite() && is_plugin_active_for_network( trailingslashit( dirname( __FILE__ ) ) . str_replace( 'class.', '', basename( __FILE__ ) ) ) )
...
<?php
		if ( $this->_is_mn_settings_page ) {
			$action = '';
			if ( isset( $_POST['collapsible_widget_options'] ) ) {
				$msg = $this->save_mn_settings( $_POST['collapsible_widget_options'] );
			}
?>
...
<?php
			...
			$action = '';
			if ( isset( $_POST['collapsible_widget_options'] ) ) {
				$msg = $this->save_network_settings( $_POST['collapsible_widget_options'] );
			}
...
<?php
		...
		if ( isset( $msg ) ) {
?>
	<p class="updated fade"><?php echo $msg ?></p>
<?php
		}
?>
